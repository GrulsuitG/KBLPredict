...
def get_gmkey_list():
    db.cursor.execute("Select gmkey, gameDate, tcodeA, tcodeH FROM game_meta_new WHERE gameDate >= 20101015;")
    ...
    for item in db.cursor.fetchall():
        ...
        gmkey_tcode_dict[item["gmkey"]] = [item['gameDate'], item["tcodeH"], item["tcodeA"]]
    ...
    for game_num in range(12, 16):
        ...
        for gmkey in gmkey_list:
            ...
            for i in range(1, 3):
                ...
                db.cursor.execute('''SELECT score, loss FROM team_record_new WHERE gmkey='{}' and tcode={}'''.format(gmkey, tcode))
                score_loss = db.cursor.fetchall()[0]
                score, loss = score_loss['score'], score_loss['loss']

                db.cursor.execute('''SELECT gmkey FROM game_meta_new WHERE gamedate <= {} and (tcodeA = {} or tcodeH = {})
                                        ORDER BY gameDate DESC LIMIT 1, {};'''.format(gameDate, tcode, tcode, game_num))

                recent_gmkey = [item['gmkey'] for item in db.cursor.fetchall()]
                if recent_gmkey:
                    recent_gmkey.reverse()
                    if len(recent_gmkey) == 1:
                        db.cursor.execute('''SELECT * FROM team_record_new WHERE tcode = {} and gmkey = '{}';'''.format(tcode, recent_gmkey[0]))
                    else:
                        db.cursor.execute('''SELECT * FROM team_record_new WHERE tcode = {} and gmkey IN {};'''.format(tcode, tuple(recent_gmkey)))

                    recent_team_record = pd.DataFrame(db.cursor.fetchall())

                    recent_team_record.drop(columns=['team_record_idx', 'gmkey', 'tcode', 'home_away'], inplace=True)
                    ...
                    temp_df.rename(columns={"score":"avg_score", "loss":"avg_loss"}, inplace=True)
        ...
        result.to_csv("recent_avg_record_ver8_{}game.csv".format(game_num), mode="w")

        #             try:
        #                 mean = pd.DataFrame(recent_team_record.mean())
        #                 mean = mean.transpose()
        #                 mean.drop(columns=['team_record_idx', 'tcode'], inplace=True)
        #                 mean.rename(columns={"score":"avg_score", "loss":"avg_loss"}, inplace=True)
        ...
        # result.to_csv("recent_avg_record_ver7_{}game.csv".format(game_num), mode="w")
        print("game {} save".format(game_num))
